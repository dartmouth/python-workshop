...
def migrateBatch(batch_file):
  with open(batch_file) as batchfile:
    ...
    for row in reader:
      ...
      glSourceRepo = glBaseUrl + "/" + group + "/" + source_repo
      ...
      if resp.status_code == 200:
        ...
        subprocess.call(["git", "clone", "--mirror", "https://" + gl_username + ":" + gl_token + "@" + glSourceRepo + ".git", source_repo + "/.git"])
        subprocess.call(["git", "config", "--bool", "core.bare", "false"], cwd="./" + source_repo)
        subprocess.call(["git", "remote", "set-url", "origin", "https://" + gh_username + ":" + gh_token + "@" + ghBaseUrl + "/" + org + "/" + target_repo], cwd="./" + source_repo)
        ...
        webhooks_resp = requests.get("https://" + glBaseUrl + "/api/v4/projects/" + group + "%2f" + source_repo + "/hooks", headers={"PRIVATE-TOKEN": gl_token})
        ...
        for webhook in webhooks_content:
          ...
          create_wh_resp = requests.post("https://api.github.com/repos/" + org + "/" + target_repo + "/hooks", json=params, headers=headers)
          if create_wh_resp.status_code == 201:
            print("Webhook created for " + target_repo)
        # delete files on disk to prevent clutter
        subprocess.call(["rm", "-rf", source_repo])

if __name__ == "__main__":
  migrateBatch(sys.argv[1])